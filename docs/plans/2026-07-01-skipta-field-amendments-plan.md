...
- Produces: `find_customer_folder(drive, root_folder_id, customer_name) -> str | None`, `ensure_customer_folder(drive, root_folder_id, customer_name) -> str` (find-or-create, keeps the customer-folder contract), `find_file_in_folder(drive, folder_id, filename) -> str | None` (webViewLink — the idempotency lookup for sign retries), `upload_pdf(drive, folder_id, filename, pdf_bytes) -> str` (webViewLink).
...
**Interfaces:**
- Consumes: `render_amendment_html`/`render_pdf` (Task 9), `ensure_customer_folder`/`find_file_in_folder`/`upload_pdf` (Task 8), `find_amendment`/`mark_signed` (Task 7).
...
- Known judgment calls encoded above: slowapi callable-limit fallback (Task 11 note), WeasyPrint Windows skip (Task 9), guard-vs-Namespace-object fallback (Task 14 Step 4), ghcr package visibility (Task 13 Step 6), sign-retry idempotency via deterministic filename + pre-upload lookup and find-or-create customer folder (Tasks 8/12, per CR #117 review).
